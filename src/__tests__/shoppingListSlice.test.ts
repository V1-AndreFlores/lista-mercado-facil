...
describe("shoppingListSlice", () => {
  ...
  it("adds an item to the active list", () => {
    const initialState = shoppingListReducer(
      undefined,
    ...
    const state = shoppingListReducer(
      initialState,
      addShoppingListItem(createItem()),
    );
  ...
  it("does not add duplicate products even when the text has different case or accents", () => {
    ...
    const withItem = shoppingListReducer(
      initialState,
      addShoppingListItem(
        createItem({ id: "item-1", name: "Leite", normalizedName: "leite" }),
      ),
    );
    ...
    const state = shoppingListReducer(
      withItem,
      addShoppingListItem(
        createItem({ id: "item-2", name: "LEÍTE", normalizedName: "leite" }),
      ),
    );
  ...
  it("toggles an item as purchased", () => {
    const withList = shoppingListReducer(
      undefined,
      setActiveList(createList()),
    );
    const withItem = shoppingListReducer(
    ...
    const state = shoppingListReducer(
      withItem,
      toggleShoppingListItemPurchased("item-1"),
    );

    expect(state.activeList?.items[0].isPurchased).toBe(true);
  });

  it("updates the item section when the user corrects the sector", () => {
    const withList = shoppingListReducer(
      undefined,
      setActiveList(createList()),
    );
    const withItem = shoppingListReducer(
      withList,
      addShoppingListItem(createItem({ sectionName: "Mercearia" })),
    );
    ...
    expect(state.activeList?.items[0].sectionName).toBe("Bebidas");
  });
